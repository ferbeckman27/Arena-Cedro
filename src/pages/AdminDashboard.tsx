...
import {
  Users, Package, BarChart3, Calendar, Settings, LogOut, ShieldCheck,
  Download, AlertOctagon, UserCheck, Star, Search, DollarSign, Clock,
  MessageSquare, AlertTriangle, FileText, TrendingUp, Info, Plus, X,
  Check, ChevronLeft, ChevronRight, Trash2, CheckCircle2, ShieldAlert,
  UserPlus, Eye, EyeOff, Circle, Lock, Activity, Megaphone,
} from "lucide-react";
...
function AdminDashboard() {
  ...
  const adminName = sessionStorage.getItem("userName") || localStorage.getItem("userName") || "Administrador";
  ...
  // Marketing - loaded from DB
  const [promoAtiva, setPromoAtiva] = useState(false);
  const [promoTexto, setPromoTexto] = useState("Promoção Relâmpago!");
  ...
  const [dadosSintetico, setDadosSintetico] = useState({
    faturamentoTotal: 0, jogosPagos: 0, mediaDiaria: 0, acessosDiarios: 0, acessosMensais: 0,
  });
  ...
  const getCorStatus = (status: string) => {
    switch (status) {
      case "reservado": case "confirmada": return "bg-red-500/5 border-red-500/20 hover:border-red-500/50";
      case "pendente": return "bg-yellow-500/5 border-yellow-500/20 hover:border-yellow-500/50";
      default: return "bg-[#0c120f] border-white/5 hover:border-[#22c55e]/50 shadow-xl";
    }
  };

  const getCorStatusBadge = (status: string) => {
    switch (status) {
      case "reservado": case "confirmada": return "bg-red-500/10 text-red-500 border-red-500/20";
      case "pendente": return "bg-yellow-500/10 text-yellow-500 border-yellow-500/20";
      default: return "bg-[#22c55e]/10 text-[#22c55e] border-[#22c55e]/20";
    }
  };
  ...
  const carregarDadosIniciais = async () => {
    ...
    // Load promo from DB
    const { data: promoConfigs } = await supabase.from("configuracoes").select("chave, valor").in("chave", ["promo_ativa", "promo_texto", "promo_link"]);
    if (promoConfigs) {
      promoConfigs.forEach(c => {
        if (c.chave === "promo_ativa") setPromoAtiva(c.valor === "true");
        if (c.chave === "promo_texto") setPromoTexto(c.valor || "Promoção Relâmpago!");
        if (c.chave === "promo_link") setPromoLink(c.valor || "");
      });
    }
    const { data: deps } = await supabase.from("depoimentos").select("*").order("data_publicacao", { ascending: false });
    if (deps) {
      setDepoimentos(deps.map((d) => ({
        id: d.id, autor: d.autor, texto: d.comentario,
  ...
  const carregarStatsReais = async () => {
    ...
    const { data: reservasMes } = await supabase.from("reservas").select("valor_total, status, pago").gte("data_reserva", mesInicio).lte("data_reserva", mesFim);
    const receitaMensal = reservasMes?.filter(r => r.status === "confirmada" || r.pago).reduce((acc, r) => acc + Number(r.valor_total || 0), 0) || 0;
    const { data: reservasHoje } = await supabase.from("reservas").select("id").eq("data_reserva", mesFim).in("status", ["confirmada", "pendente"]);
    const ocupacao = reservasHoje ? Math.round((reservasHoje.length / 26) * 100) : 0;
    const { count: vipCount } = await supabase.from("clientes").select("id", { count: "exact", head: true }).eq("tipo", "mensalista");
    const { count: alertasCount } = await supabase.from("reservas").select("id", { count: "exact", head: true }).eq("status", "pendente").eq("pago", false);
    ...
    setStatsReais({ receitaMensal, ocupacao, clientesVip: vipCount || 0, alertas: alertasCount || 0, acessosDiarios });
  };
  ...
  const carregarDadosSintetico = async () => {
    const { data: reservas } = await supabase.from("reservas").select("valor_total, status, pago, data_reserva").gte("data_reserva", sinteticoInicio).lte("data_reserva", sinteticoFim);
    ...
    setDadosSintetico({ faturamentoTotal, jogosPagos: reservasPagas.length, mediaDiaria: faturamentoTotal / diasPeriodo, acessosDiarios, acessosMensais });
  };

  const carregarCaixa = async () => {
    const { data: reservasDia } = await supabase.from("reservas").select("valor_total, forma_pagamento, pago, status, valor_restante").eq("data_reserva", caixaData);
  ...
  useEffect(() => { carregarDadosSintetico(); }, [sinteticoInicio, sinteticoFim]);
  useEffect(() => { carregarCaixa(); }, [caixaData]);
  useEffect(() => {
    const pendentes = depoimentos.filter((d) => d.status === "pendente").length;
    if (pendentes > refPendentesDepoimentos.current) { playApito(); refPendentesDepoimentos.current = pendentes; }
  ...
  const gerarEstruturaAgenda = (duracaoMinutos: number) => {
    ...
    for (const periodo of periodos) {
      ...
      while (atual + duracaoMinutos / 60 <= periodo.fim) {
        ...
        slots.push({ inicio, fim, turno: horas >= 18 ? "noturno" : "diurno", status: "livre" });
        atual += 0.5;
      }
    }
    return slots;
  ...
  const salvarProduto = async () => {
    const payload: any = {
      nome: formProduto.nome, tipo: formProduto.tipo,
    ...
    if (editandoProduto) { await supabase.from("produtos").update(payload).eq("id", editandoProduto.id); }
    else { await supabase.from("produtos").insert([payload]); }
    setModalProdutoAberto(false); carregarProdutos();
    toast({ title: "Produto salvo!" });
  };

  const excluirProduto = async (id: number) => {
    if (confirm("Excluir produto?")) { await supabase.from("produtos").delete().eq("id", id); carregarProdutos(); }
  };
  ...
  const handleToggleStatusEquipe = async (id: string, statusAtual: boolean) => {
    if (confirm(`Deseja ${statusAtual ? "DESATIVAR" : "ATIVAR"} este funcionário?`)) {
      ...
      if (error) { toast({ variant: "destructive", title: "Erro na operação", description: error.message }); }
      else { toast({ title: statusAtual ? "Funcionário Desativado" : "Funcionário Ativado" }); setListaEquipe((prev) => prev.map((f) => (f.id === id ? { ...f, ativo: !statusAtual } : f))); }
    }
  };
  ...
  const funcPasswordValidations = useMemo(() => {
    ...
    return { hasExactLength, hasUpperCase, hasLowerCase, hasSpecialChar, hasNumber, noRepeatedNumbers,
  ...
  const handleCadastrarFuncionario = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!funcPasswordValidations.isValid) return toast({ variant: "destructive", title: "Senha Inválida" });
    if (!novoFuncEmail.toLowerCase().endsWith("@atendcedro.com")) return toast({ variant: "destructive", title: "E-mail Inválido", description: "O e-mail deve terminar com @atendcedro.com" });
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        email: emailFinal, password: novoFuncSenha,
  ...
  const baixarPdfSintetico = async () => {
    try {
      ...
      const diasPeriodo = Math.max(1, Math.ceil((d2.getTime() - d1.getTime()) / (1000 * 60 * 60 * 24)) + 1);
      const doc = new jsPDF();
      doc.setFontSize(20); doc.setFont(undefined!, "bold"); doc.text("RELATÓRIO SINTÉTICO - ARENA CEDRO", 20, 25);
  ...
  const baixarPdfAnalitico = async () => {
    try {
      ...
      const diasPeriodo = Math.max(1, Math.ceil((d2.getTime() - d1.getTime()) / (1000 * 60 * 60 * 24)) + 1);
      const taxaOcupacao = reservas?.length ? Math.min(100, ((reservas?.length || 0) / (diasPeriodo * 26)) * 100) : 0;
      const doc = new jsPDF();
      doc.setFontSize(18); doc.setFont(undefined!, "bold"); doc.text("RELATÓRIO ANALÍTICO - ARENA CEDRO", 20, 20);
      doc.setFontSize(10); doc.setFont(undefined!, "normal"); doc.setTextColor(100);
  ...
  const slotsCalculados = useMemo(() => {
    ...
    return gradeMestre.map((slotVazio) => {
      ...
      if (reservaReal) {
        ...
        return { ...slotVazio, status: slotStatus, valor: reservaReal.valor_total || valorPadrao,
          reserva: { cliente: reservaReal.cliente_nome, pagamento: reservaReal.forma_pagamento, obs: reservaReal.observacoes, tipo: reservaReal.tipo, pago: reservaReal.pago, valor_total: reservaReal.valor_total, valor_sinal: reservaReal.valor_sinal } };
      }
  ...
  const abrirDetalheSlot = (slot: any) => {
    if (slot.status === "reservado" || slot.status === "pendente") {
      setSlotDetalhe({
        inicio: slot.inicio, fim: slot.fim, cliente: slot.reserva?.cliente || "Não informado",
        pagamento: slot.reserva?.pagamento || "Pendente", obs: slot.reserva?.obs || "",
        valor: slot.valor, tipo: slot.reserva?.tipo || "avulsa", pago: slot.reserva?.pago || false,
        valorSinal: slot.reserva?.valor_sinal || 0, valorTotal: slot.reserva?.valor_total || slot.valor,
        status: slot.status,
      });
      setIsModalDetalheAberto(true);
    } else {
      toast({ title: "Horário Livre", description: `O horário das ${slot.inicio} está disponível.` });
    }
  };

  const handlePagarVip = async (id: number) => {
    const { error } = await supabase.from("clientes").update({ status_pagamento: "em_dia" }).eq("id", id);
    if (!error) { setVipsReais((prev) => prev.map((v: any) => (v.id === id ? { ...v, status_pagamento: "em_dia" } : v))); toast({ title: "Mensalidade confirmada." }); }
    else { toast({ variant: "destructive", title: "Erro ao confirmar mensalidade." }); }
  };

  // Save marketing to DB instead of localStorage
  const salvarPromocao = async () => {
    await supabase.from("configuracoes").update({ valor: String(promoAtiva) }).eq("chave", "promo_ativa");
    await supabase.from("configuracoes").update({ valor: promoTexto }).eq("chave", "promo_texto");
    await supabase.from("configuracoes").update({ valor: promoLink }).eq("chave", "promo_link");
  ...
  const carregarComissoes = async () => {
    ...
    if (data) {
      ...
      const { data: funcs } = await supabase.from("funcionarios").select("id, nome, sobrenome, turno").in("id", ids.length > 0 ? ids : ["none"]);
      const funcMap: Record<string, any> = {};
      funcs?.forEach(f => { funcMap[f.id] = { nome: `${f.nome} ${f.sobrenome || ""}`.trim(), turno: f.turno || "—" }; });
    ...
    <div className="min-h-screen bg-[#060a08] text-white font-sans pb-10">
      <header className="w-full bg-[#0c120f] border-b border-white/5 px-6 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-center">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[20px] font-black uppercase text-[#22c55e] tracking-[0.2em]">Bem Vindo, {adminName}</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-4">
            ...
            <Button variant="ghost" className="text-red-500" onClick={handleLogout}><LogOut size={20} /></Button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-8 space-y-8">
        ...
        <Tabs defaultValue="agenda" className="space-y-6">
                  ...
                  {slotsCalculados.map((slot, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => abrirDetalheSlot(slot)} className={cn("p-5 rounded-[2rem] border text-center transition-all relative overflow-hidden group flex flex-col items-center justify-center gap-1", getCorStatus(slot.status))}>
                        ...
                        <p className="text-xl font-black italic text-white group-hover:scale-110 transition-transform">{slot.inicio}-{slot.fim}</p>
                        {/* Badge tipo (avulsa/fixa) */}
                        {isOcupado && slot.reserva?.tipo && (
                          <Badge className={cn("text-[7px] font-black border-none mt-1", slot.reserva.tipo === "fixa" ? "bg-purple-500/20 text-purple-400" : "bg-blue-500/20 text-blue-400")}>
                            {slot.reserva.tipo === "fixa" ? "FIXA/VIP" : "AVULSA"}
                          </Badge>
                        )}
                        <div className={cn("mt-1 px-3 py-0.5 rounded-full text-[8px] font-black uppercase border", getCorStatusBadge(slot.status))}>
                    ...
                    <div className="space-y-6 pt-6">
                      ...
                      <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-2">
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              <p className="text-2xl font-black italic text-[#22c55e]">R$ {(valorPago > 0 ? valorPago : valorTotal).toFixed(2)}</p>
                              {teveDesconto ? (
                                <Badge className="bg-[#22c55e]/10 text-[#22c55e] border-none text-[9px] font-black">✅ COM DESCONTO PIX (-R$ {descontoAplicado.toFixed(2)})</Badge>
                              ) : (
                                <Badge className="bg-gray-500/10 text-gray-400 border-none text-[9px] font-black">SEM DESCONTO (VALOR INTEGRAL)</Badge>
                              )}
                            </>
                          );
                        })()}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <Badge className={cn("text-[9px] font-black border-none", slotDetalhe.tipo === 'fixa' ? "bg-purple-500/20 text-purple-400" : "bg-blue-500/20 text-blue-400")}>
                            {slotDetalhe.tipo === 'fixa' ? 'FIXA / VIP' : 'AVULSA'}
                          </Badge>
                        </div>
                        <div className="flex justify-between text-sm">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <Card className="bg-[#0c120f] border-white/10 p-8 rounded-[3rem] border-t-4 border-t-[#22c55e]">
                <div className="flex justify-between items-start mb-6">
                  <div><h3 className="text-2xl font-black italic uppercase">Sintético</h3><p className="text-gray-500 text-xs italic">Resumo de desempenho</p></div>
                  <BarChart3 className="text-[#22c55e]" size={32} />
                </div>
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Início</Label><Input type="date" value={sinteticoInicio} onChange={e => setSinteticoInicio(e.target.value)} className="bg-white/5 border-white/10 text-white mt-1" /></div>
                  <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Final</Label><Input type="date" value={sinteticoFim} onChange={e => setSinteticoFim(e.target.value)} className="bg-white/5 border-white/10 text-white mt-1" /></div>
                </div>
                <div className="space-y-4 mb-8">
                  <div className="flex justify-between p-4 bg-white/5 rounded-2xl border border-white/5"><span className="text-sm font-bold text-gray-400">Faturamento Total</span><span className="text-xl font-black text-[#22c55e]">{formatarMoeda(dadosSintetico.faturamentoTotal)}</span></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5"><p className="text-[10px] text-gray-500 font-bold uppercase">Jogos Pagos</p><p className="text-lg font-black italic">{dadosSintetico.jogosPagos}</p></div>
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5"><p className="text-[10px] text-gray-500 font-bold uppercase">Média Diária</p><p className="text-lg font-black italic">{formatarMoeda(dadosSintetico.mediaDiaria)}</p></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5"><p className="text-[10px] text-gray-500 font-bold uppercase">Acessos Diários</p><p className="text-lg font-black italic">{dadosSintetico.acessosDiarios}</p></div>
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5"><p className="text-[10px] text-gray-500 font-bold uppercase">Acessos no Período</p><p className="text-lg font-black italic">{dadosSintetico.acessosMensais}</p></div>
                  </div>
                </div>
                <Button onClick={baixarPdfSintetico} className="w-full bg-[#22c55e] text-black font-black uppercase italic h-14 rounded-2xl"><Download size={18} className="mr-2" /> Baixar PDF Sintético</Button>
              </Card>
              <Card className="bg-[#0c120f] border-white/10 p-8 rounded-[3rem] border-t-4 border-t-blue-500 shadow-2xl">
                <div className="flex justify-between items-start mb-6">
                  <div><h3 className="text-2xl font-black italic uppercase text-white">Analítico</h3><p className="text-gray-500 text-xs italic">Detalhamento por categoria</p></div>
                  <FileText className="text-blue-500" size={32} />
                </div>
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Início</Label><Input type="date" value={analiticoInicio} onChange={e => setAnaliticoInicio(e.target.value)} className="bg-white/5 border-white/10 text-white mt-1" /></div>
                  <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Final</Label><Input type="date" value={analiticoFim} onChange={e => setAnaliticoFim(e.target.value)} className="bg-white/5 border-white/10 text-white mt-1" /></div>
                </div>
                <div className="space-y-3 mb-6"><p className="text-[10px] text-gray-500 font-bold uppercase">Os dados serão calculados ao baixar o PDF</p></div>
                <Button onClick={baixarPdfAnalitico} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black uppercase italic h-14 rounded-2xl flex gap-2"><FileText size={18} /> Baixar PDF Analítico</Button>
              </Card>
            </div>
          </TabsContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {produtos.map((p) => (
                  <Card key={p.id} className="bg-[#0c120f] border-white/5 p-6 rounded-[2.5rem] relative overflow-hidden group">
                    <Badge className={cn("absolute top-4 right-4 font-black italic", p.tipo === "venda" ? "bg-blue-500/20 text-blue-400" : p.tipo === "aluguel" ? "bg-purple-500/20 text-purple-400" : "bg-orange-500/20 text-orange-400")}>{String(p.tipo).toUpperCase()}</Badge>
                    <div className="mt-4">
                      <p className="text-xl font-black italic uppercase text-white">{p.nome}</p>
                      <div className="mt-6 flex flex-col gap-3">
                        <div className="flex justify-between items-end">
                          <div className="space-y-2">
                            {(p.tipo === "venda" || p.tipo === "ambos") && (<div><p className="text-[10px] font-black text-gray-500 uppercase leading-none">Venda</p><p className="text-xl font-black text-[#22c55e]">R$ {Number(p.preco_venda || p.preco || 0).toFixed(2)}</p></div>)}
                            {(p.tipo === "aluguel" || p.tipo === "ambos") && (<div><p className="text-[10px] font-black text-gray-500 uppercase leading-none">Aluguel</p><p className="text-xl font-black text-purple-400">R$ {Number(p.preco_aluguel || 0).toFixed(2)}</p></div>)}
                          </div>
              ...
              {["pendente", "aprovado"].map((tipo) => (
                <Card key={tipo} className="bg-[#0c120f] border-white/5 p-8 rounded-[2.5rem] min-h-[600px] flex flex-col">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <Badge className={cn("font-black px-3 py-1 rounded-full text-[10px] border-none", tipo === "pendente" ? "bg-yellow-500/10 text-yellow-500" : "bg-[#22c55e]/10 text-[#22c55e]")}>{depoimentos.filter((d) => d.status === tipo).length} ITENS</Badge>
                  </div>
                    ...
                    <div className="space-y-4">
                      {depoimentos.filter((d) => d.status === tipo).length === 0 ? (
                        ...
                        depoimentos.filter((d) => d.status === tipo).map((d) => (
                          <div key={d.id} className="p-5 bg-white/5 border border-white/10 rounded-[2rem] group hover:border-[#22c55e]/30 transition-all">
                            <div className="flex justify-between items-start mb-3">
                              <div>
                                <p className="text-[10px] text-[#22c55e] font-black uppercase">{d.autor}</p>
                                <p className="text-[9px] text-gray-600 font-bold mt-1 uppercase">{d.data}</p>
                              </div>
                              <Button variant="ghost" size="icon" onClick={() => processarComentario(d.id, "rejeitado")} className="h-8 w-8 text-red-500 hover:bg-red-500/10 rounded-full opacity-0 group-hover:opacity-100"><Trash2 size={14} /></Button>
                            </div>
                            <p className="text-sm italic text-gray-300 leading-relaxed mb-5">"{d.texto}"</p>
                            <div className="flex gap-2">
                              {tipo === "pendente" ? (
                                <Button onClick={() => processarComentario(d.id, "aprovado")} className="w-full bg-[#22c55e] text-black font-black text-[10px] uppercase h-10 rounded-xl"><Check size={14} className="mr-2" /> Aprovar</Button>
                              ) : (
                                <Button onClick={() => processarComentario(d.id, "pendente")} variant="outline" className="w-full border-white/10 text-gray-500 font-black text-[10px] uppercase h-10 rounded-xl">Remover do Mural</Button>
                              )}
                            </div>
                          </div>
                        ))
          ...
          <TabsContent value="financeiro">
            <div className="space-y-8">
              ...
              <Card className="bg-[#0c120f] border-white/10 p-8 rounded-[3rem]">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                  <h2 className="text-xl font-black italic uppercase text-white flex items-center gap-2"><DollarSign className="text-[#22c55e]" /> Relatório de Comissões</h2>
                </div>
                <div className="grid grid-cols-2 gap-3 mb-6 max-w-md">
                  <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Início</Label><Input type="date" value={comissaoInicio} onChange={e => setComissaoInicio(e.target.value)} className="bg-white/5 border-white/10 text-white mt-1" /></div>
                  <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Final</Label><Input type="date" value={comissaoFim} onChange={e => setComissaoFim(e.target.value)} className="bg-white/5 border-white/10 text-white mt-1" /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {relatorioComissoes.map((item: any, index) => (
          ...
          {/* MARKETING - saves to DB */}
          <TabsContent value="marketing">
            <Card className="bg-[#0c120f] border-white/10 p-8 rounded-[3rem] space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-black italic uppercase flex items-center gap-3"><Megaphone className="text-[#22c55e]" /> Promoção (Popup na Página Principal)</h3>
                <Switch checked={promoAtiva} onCheckedChange={setPromoAtiva} />
              </div>
              <p className="text-xs text-gray-500">Quando ativado, um popup aparecerá na página principal para todos os visitantes.</p>
              <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Texto da Promoção</Label><Textarea value={promoTexto} onChange={(e) => setPromoTexto(e.target.value)} className="bg-white/5 border-white/10 h-24 italic font-bold mt-1" placeholder="Texto da Promoção" /></div>
              <div><Label className="text-[10px] uppercase text-gray-500 font-bold">Link do Botão (ex: /login, https://...)</Label><Input value={promoLink} onChange={(e) => setPromoLink(e.target.value)} className="bg-white/5 border-white/10 mt-1" placeholder="Link do Botão" /></div>
              <Button onClick={salvarPromocao} className="w-full bg-[#22c55e] text-black font-black uppercase italic h-14 rounded-2xl">Publicar Promoção</Button>
            </Card>
          </TabsContent>
              ...
              <Table>
                <TableHeader className="bg-white/5">
                  <TableRow className="border-white/5 text-[10px] uppercase font-black">
                    <TableHead>Atendente</TableHead><TableHead>E-mail</TableHead><TableHead>Acessos</TableHead><TableHead>Último Acesso</TableHead><TableHead>Turno</TableHead><TableHead className="text-right">Gestão</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {listaEquipe.length > 0 ? (
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <Button variant="ghost" size="sm" className="h-8 rounded-xl border border-red-500/20 text-red-500 hover:bg-red-500/10" onClick={async () => {
                              if (confirm(`Deseja EXCLUIR permanentemente ${membro.nome} ${membro.sobrenome || ''}?`)) {
                                ...
                                else { toast({ title: "Funcionário excluído!" }); setListaEquipe((prev) => prev.filter((f: any) => f.id !== membro.id)); }
                              }
                            }}>
                              <Trash2 size={14} /><span className="ml-1 text-[9px] font-black uppercase">Excluir</span>
                            </Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ...
                    <TableRow><TableCell colSpan={6} className="text-center py-10 opacity-50 italic">Nenhum atendente cadastrado...</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </Card>
          ...
          <TabsContent value="cadastro">
            <Card className="bg-[#0c120f] border-white/5 p-8 rounded-[2.5rem] max-w-2xl mx-auto">
              <div className="flex items-center gap-3 mb-8">
                ...
                <div><h3 className="text-2xl font-black italic uppercase text-white">Cadastrar Funcionário</h3><p className="text-gray-500 text-xs">Preencha os dados abaixo</p></div>
              </div>
              <form onSubmit={handleCadastrarFuncionario} className="space-y-5">
                ...
                <Button type="submit" disabled={loadingCadastro || !funcPasswordValidations.isValid} className="w-full bg-[#22c55e] text-black font-black uppercase italic h-14 rounded-2xl disabled:opacity-30">
